...
sub ftp_get_pre {
	...
	foreach my $proteome (@pep_listing) {
		...
		if ($proteome =~ m/\.fa\z/) {
			#$ftp->cdup();
			$log->warn("Report: working with RAW fasta .prep.pep.fa file with TAX_ID:$tax_id");
			#last FILE;
			get_raw_fasta( { %{$param_href}, SPEC_PATH => $spec_path, TAXID => $tax_id, PROTEOME => $proteome, FTP => $ftp } );
			next FILE;
		}
